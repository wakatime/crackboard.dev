...
export default function TopUsersList({ languageName }: { languageName: string }) {
  const { ref, inView } = useInView();
  const topUsersQuery = api.leaders.topUsersForLanguage.useInfiniteQuery(
    { languageName },
    {
  ...
  useEffect(() => {
    if (inView && topUsersQuery.hasNextPage && !topUsersQuery.isFetching) {
