...
export default function IntegrationLeadersList() {
  const query = api.leaders.getLanguagesWithLeaders.useQuery();
  ...
  return (
    <div className="grid gap-4 p-4 sm:grid-cols-2">
      {query.data
        ...
        .map((i) => {
          return (
            <Card className="hover:bg-muted" key={i.id}>
              <Link href={`/leaders/language/${i.id}`}>
                <div className="items-top flex gap-0 p-4">
                  <div className="flex flex-col items-center gap-1">
                    <div className="h-10 w-10 rounded-full border" style={{ backgroundColor: i.language.color ?? undefined }} />
                    <div className="bg-border h-16 w-px" />
                  </div>
                  <div className="flex flex-1 flex-col overflow-hidden">
                    <h3 className="mb-2 mt-2 truncate pl-3 font-semibold leading-6">{i.language.name}</h3>
                    <ul className="list-outside list-disc pl-5 align-middle">
                      {i.users.map((user, index) => {
                        ...
                        return (
                          <li className={`relative mb-1 ${opacity}`} key={`${i.language.name}${user.user.id}`}>
