...
export const getLeaderboardConfig = async () => {
  const [config] = await db.select().from(LeaderboardConfig).where(eq(LeaderboardConfig.id, LEADERBOARD_CONFIG_ID));
  if (config) {
    return config;
  }
  const [newConfig] = await db.insert(LeaderboardConfig).values({ id: LEADERBOARD_CONFIG_ID }).returning();
  if (!newConfig) {
